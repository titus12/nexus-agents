    ...
    def build(
        ...
        *,
        ...
        if (
            ...
            prompt = (
                f"{prompt.rstrip()}\n\n"
                "TRANSPORT CONTRACT (authoritative): write the one complete "
                f"business JSON result to {result_path} as UTF-8 without BOM. "
                "Use a real JSON serializer so embedded quotes and control "
                "characters are escaped. Include the exact root field "
                ...
                "or replace only this result file. Do not paste the business "
                "result into the issue comment; after the file is durably "
                "written, reply only with the compact result pointer. "
