...
4. **任务必须有边界。** 每个任务都必须声明目标、输入、允许修改范围、完成标准、验证和风险。超过预算的任务必须先拆分，不能直接交给一个 subagent。
5. **workflow 选择授权角色编排。** `$wf-go-feat` 和 `$wf-go-bugfix` 默认按角色使用 subagent；主线程始终负责拆分、集成、最终 diff 审查和最终验证。
