...
func BuildInstructions(input CompileInput) string {
	...
	builder.WriteString("- `openwiki/index.md` is the project navigation page and must not use YAML frontmatter.\n")
	...
	builder.WriteString("- First identify the project's stable core capabilities, then create one canonical page per capability at `openwiki/domains/<stable-domain-slug>/index.md`.\n")
	...
	builder.WriteString("- Every Domain index must contain substantive knowledge: responsibility/boundary, important entrypoints, owned data or state, related Domains, source paths, and verification. Never emit an empty index or a generic `Files and subdirectories` page.\n")
	...
	builder.WriteString("- `openwiki/index.md` must list only the direct Domain pages so a developer can immediately see the project's core functions.\n")
	builder.WriteString("- Do not create `openwiki/domains/index.md`, a Domain named `domains` or `index`, or links to Domain directories without an explicit `index.md` target.\n")
	...
	builder.WriteString("- Every `domains/<slug>/index.md` page must use `type: Domain`, a stable English slug, tags, a concise title/description, and explicit Markdown links to related Domains.\n")
